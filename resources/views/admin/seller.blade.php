        ...
        @if (count($sellers) > 0)
            <ul class="cart__item-list">
                @foreach ($sellers as $s)
                    <li class="cart__item">
                        <h1>{{ $s->name }}</h1>
                        <h2>Total Products: {{ $s->total_products }}</h2>
                        <form action="/deleteSeller/{{$s->id}}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button class="btn danger" type="submit">Delete Seller</button>
                        </form>
                    </li>
                @endforeach
            </ul>
